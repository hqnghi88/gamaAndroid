/*********************************************************************************************
 *
 * 'TextLayerStatement.java, in plugin msi.gama.core, is part of the source code of the GAMA modeling and simulation
...
@facets (
		value = { @facet (
				...
				// 10/04/13 Name is not a constant ID anymore but can represent the text
				// to display.
...
@doc (
		deprecated = "Use a 'graphics' type of layer instead, with a `draw text ` statement inside",
		value = "`" + IKeyword.TEXT
		...
		usages = { @usage (
				...
				examples = { @example (
						...
						isExecutable = false),
						@example (
								value = "   text expression [additional options];",
								isExecutable = false),
						@example (
								value = "}",
								isExecutable = false) }),
...
public class TextLayerStatement extends AbstractLayerStatement {
	...
	public TextLayerStatement(final IDescription desc) throws GamaRuntimeException {
		...
		if (getFacet(IKeyword.SIZE) == null) {
			final IExpression f = font;
			final IExpressionFactory ef = GAML.getExpressionFactory();
			final IExpression s = ef.createOperator("as", desc, desc.getUnderlyingElement(IKeyword.FONT), f,
					ef.createTypeExpression(Types.INT));
			final IExpression exp = ef.createOperator("*", desc, null, s, new PixelUnitExpression("px", null));
	...
	public boolean _step(final IScope scope) {
		...
		if (style != null) {
			final int currentStyle = CONSTANTS.get(Cast.asString(scope, style.value(scope)));
