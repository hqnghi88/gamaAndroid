...
@facets(value = {
		...
		@facet(name = IKeyword.ITEM, type = IType.NONE, optional = true, doc = {
				@doc("any expression to add in the container") }),
		@facet(name = IKeyword.EDGE, type = IType.NONE, optional = true, doc = {
				@doc(deprecated = "Use 'add edge(...)' instead", value = "a pair that will be added to a graph as an edge (if nodes do not exist, they are also added)") }),
		@facet(name = IKeyword.VERTEX, type = IType.NONE, optional = true, doc = {
				@doc(deprecated = "Use 'add node(...)' instead") }),
		@facet(name = IKeyword.NODE, type = IType.NONE, optional = true, doc = {
				@doc(deprecated = "Use 'add node(...)' instead", value = "an expression that will be added to a graph as a node.") }),
		@facet(name = IKeyword.AT, type = IType.NONE, optional = true, doc = {
				@doc("position in the container of added element") }),
		@facet(name = IKeyword.ALL, type = IType.NONE, optional = true, doc = @doc("Allows to either pass a container so as to add all its element, or 'true', if the item to add is already a container.")),
		@facet(name = IKeyword.WEIGHT, type = IType.FLOAT, optional = true) }, omissible = IKeyword.ITEM)
@doc(value = "Allows to add, i.e. to insert, a new element in a container (a list, matrix, map, ...).Incorrect use: The addition of a new element at a position out of the bounds of the container will produce a warning and let the container unmodified. If all: is specified, it has no effect if its argument is not a container, or if its argument is 'true' and the item to add is not a container. In that latter case", usages = {
		@usage(value = "The new element can be added either at the end of the container or at a particular position.", examples = {
		...
		@usage(value = "Case of a list, the expression in the facet at: should be an integer.", examples = {
				@example("list<int> workingList <- [];"),
				@example(value = "add 0 at: 0 to: workingList ;", var = "workingList", equals = "[0]", returnType = "null"), // workingList
																																// now
																																// equals
																																// [0]
				@example(value = "add 10 at: 0 to: workingList ;", var = "workingList", equals = "[10,0]", returnType = "null"), // workingList
																																	// now
																																	// equals
																																	// [10,0]
				@example(value = "add 20 at: 2 to: workingList ;", var = "workingList", equals = "[10,0,20]", returnType = "null"), // workingList
																																	// now
																																	// equals
																																	// [10,0,20]
				@example(value = "add 50 to: workingList;", var = "workingList", equals = "[10,0,20,50]", returnType = "null"), // workingList
																																// now
																																// equals
																																// [10,0,20,50]
				@example(value = "add [60,70] all: true to: workingList;", var = "workingList", equals = "[10,0,20,50,60,70]", returnType = "null") }), // workingList
																																						// now
																																						// equals
																																						// [10,0,20,50,60,70]
		...
		@usage(value = "Case of a map: As a map is basically a list of pairs key::value, we can also use the add statement on it. "
				+ "It is important to note that the behavior of the statement is slightly different, in particular in the use of the at facet, which denotes the key of the pair.", examples = {
						@example("map<string,string> workingMap <- [];"),
						@example(value = "add \"val1\" at: \"x\" to: workingMap;", var = "workingMap", equals = "[\"x\"::\"val1\"]", returnType = "null") }), // workingMap
																																								// now
																																								// equals
																																								// [x::val1]";
		@usage(value = "If the at facet is omitted, a pair expr_item::expr_item will be added to the map. "
				+ "An important exception is the case where the expr_item is a pair: in this case the pair is added.", examples = {
						@example(value = "add \"val2\" to: workingMap;", var = "workingMap", equals = "[\"x\"::\"val1\", \"val2\"::\"val2\"]", returnType = "null"), // workingMap
																																										// now
																																										// equals
																																										// [val2::val2,
																																										// x::val1]
						@example(value = "add \"5\"::\"val4\" to: workingMap; ", var = "workingMap", equals = "[\"x\"::\"val1\", \"val2\"::\"val2\", \"5\"::\"val4\"]", returnType = "null") }), // workingMap
																																																	// now
																																																	// equals
																																																	// [val2::val2,
																																																	// 5::val4,
		...
		@usage(value = "Notice that, as the key should be unique, the addition of an item at an existing position (i.e. existing key) "
				+ "will only modify the value associated with the given key.", examples = {
						@example(value = "add \"val3\" at: \"x\" to: workingMap;", var = "workingMap", equals = "[\"x\"::\"val3\", \"val2\"::\"val2\", \"5\"::\"val4\"]", returnType = "null") }), // workingMap
																																																	// now
																																																	// equals
																																																	// [x::val3,
																																																	// val2::value2,
		...
		@usage(value = "On a map, the all facet will add all value of a container  in the map (so as pair val_cont::val_cont)", examples = {
				@example(value = "add [\"val4\",\"val5\"] all: true at: \"x\" to: workingMap;", var = "workingMap", equals = "[\"x\"::\"val3\", \"val2\"::\"val2\", \"5\"::\"val4\",\"val4\"::\"val4\",\"val5\"::\"val5\"]", returnType = "null") }), // workingMap
																																																														// now
																																																														// equals
																																																														// [x::val3,
																																																														// val2::value2,
		...
		@usage(value = "In case of a graph, we can use the facets `node`, `edge` and `weight` to add a node, an edge or weights to the graph. However, these facets are now considered as deprecated, and it is advised to use the various edge(), node(), edges(), nodes() operators, which can build the correct objects to add to the graph ", examples = {
				...
				@example(value = "g.edges", returnType = IKeyword.LIST, equals = "[polyline({1.0,5.0}::{12.0,45.0}),polyline({1.0,5.0}::{2.0,3.0})]"),
				...
				@example(value = "g.edges", returnType = IKeyword.LIST, equals = "[polyline({1.0,5.0}::{12.0,45.0}),polyline({1.0,5.0}::{2.0,3.0})]") }) }, see = {
						"put", "remove" })
@symbol(name = IKeyword.ADD, kind = ISymbolKind.SINGLE_STATEMENT, with_sequence = false, concept = { IConcept.CONTAINER,
		IConcept.GRAPH, IConcept.NODE, IConcept.EDGE, IConcept.MAP, IConcept.MATRIX, IConcept.LIST })
